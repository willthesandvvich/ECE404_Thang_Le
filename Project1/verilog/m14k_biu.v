...
module m14k_biu(
	...
	output 		HMASTLOCK;	//AHB: Indicates the current transfer is part of a locked sequence; 
                    ...
                    //AHB: since not using AHB-Lite, this is routed to HLOCKx in arbiter
...
wire hbreq;
    ...
    mvp_cregister #(1) _hgrant_reg(hgrant_reg, if_enable, gclk, ~greset & HGRANT);

    // Assert a request for the bus if there is a request for data from either
    // cache controller
    assign hbreq = (ireq_val | dreq_val | wrreq_val) | hbreq_reg;
    // De-assert request after acquiring a grant (unless there is another
    // request from a cache controller)
    mvp_register #(1) _hbreq_reg(hbreq_reg, gclk, ~greset & (hbreq & ~HGRANT));

    assign HBUSREQ = hbreq;
...
// sync on uncached stores to flush them out of the write thru buffer
    ...
    // Is there a hit in the (front) write buffer?
    // This occurs when the address field (31:4) match, and when the tag in
    // the write buffer is valid (field 0) == 1).
...
//
	...
	assign ireq_st0 = (HGRANT && idle_st && ireq_val && ~(wrreq_val && sync_hold) && !sync_retain && !(read_lock || lock_hold) && !wreq_redo_pre) 
	...
	// ireq_st0 would be restart when it was terminated by ERROR response of prev one 
	assign dreq_st0 = (HGRANT && idle_st && !ireq_val && !wrreq_val && dreq_val && !(ld_hit_flush || hold_flush_wrb) && !wreq_redo_pre) || 
	...
	assign wreq_sta0 = (idle_st && (HGRANT||hgrant_reg) && !(ireq_val && !sync_retain && !(read_lock || lock_hold) && !sync_hold) && 
